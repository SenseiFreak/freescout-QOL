...
class CalendarProvider
{
    ...
    public function findGoogleTicketEvents($userId, $calendar, $ticketUrl)
    {
        $items = $this->pages('google', $userId, 'calendars/'.rawurlencode($calendar).'/events', [
            'q' => $ticketUrl, 'maxResults' => 250, 'showDeleted' => 'false',
        ]);
        return array_values(array_filter($items, function ($item) use ($ticketUrl) {
            // Google full-text search is broad: verify the exact ticket URL and
            // its boundary, so ticket 42 cannot match ticket 420.
            return ($item['status'] ?? '') !== 'cancelled' && !empty($item['id'])
                && preg_match('~'.preg_quote($ticketUrl, '~').'(?=$|[\s<>"\x27?&#])~', html_entity_decode($item['description'] ?? '', ENT_QUOTES));
        }));
    }
