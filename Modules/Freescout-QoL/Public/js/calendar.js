(function () {
    ...
    async function changeSource(preferredCalendar) {
        var current = ++generation;
        events = []; render(); calendar.replaceChildren(); calendar.disabled = true;
        if (source.value === 'local') { loadingCalendars = false; calendar.add(new Option('Personal calendar', '')); await refresh(); return true; }
        ...
        try {
            ...
            calendar.disabled = false; loadingCalendars = false;
            if (typeof preferredCalendar === 'string' && preferredCalendar) {
                calendar.value = preferredCalendar;
                if (!calendar.value) {
                    var missing = new Option('Saved calendar unavailable — choose a calendar', '');
                    missing.disabled = true; missing.selected = true; calendar.add(missing, 0);
                    status.textContent = 'Your default calendar is unavailable. Select another calendar or update your default in QoL settings.';
                    return false;
                }
            }
            await refresh(); return !!calendar.value;
        } catch (e) { if (current === generation) { loadingCalendars = false; status.textContent = e.message; } return false; }
    }
    function edit(event, at) {
        ...
        document.getElementById('qc-destination').textContent = source.value === 'local' ? 'Saved in your FreeScout calendar. Linked event details are also visible to agents who can view the ticket.' : 'Saved directly to ' + calendar.selectedOptions[0].text + '. Ticket links and notes will be visible to people with access to that calendar. Linked events are also recorded on the ticket.';
        dialog.showModal();
    }
    form.onsubmit = async function (e) {
        ...
        try {
            var result = await request(root.dataset.save, data);
    ...
    view.onchange = refresh; source.onchange = function () { changeSource(); }; calendar.onchange = refresh;
    async function initialize() {
        var preferred = root.dataset.defaultSource || 'local';
        var available = Array.prototype.some.call(source.options, function (option) { return option.value === preferred; });
        source.value = available ? preferred : 'local';
        render();
        var ready = await changeSource(root.dataset.defaultCalendar || '');
        if (!available) {
            status.textContent = 'Your default calendar account is disconnected. Showing FreeScout; reconnect the account or update your default in QoL settings.';
            return;
        }
        if (root.dataset.ticket && ready) edit();
    }
    initialize();
